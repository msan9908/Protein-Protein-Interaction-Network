...
def add_node(G, m, n, method='random', alpha=1.0):
    ...
    if method == 'bio_smart':
        ...
        return G
def add_mutant_node(G,  method='random', alpha=1.0):
    """
    Add node to the network according to the method supplied. This new node may
    be added randomly, preferentially based on degree, or using insights about
    empirical distributions of protein-specific interaction patterns (here
    ...
    print(eijs)
    duplicate_node = G.nodes[eijs].copy()
    new_node_id = max(G.nodes) + 1
    G.add_node(new_node_id, **duplicate_node)
    edges_to_remove = np.random.choice(list(G.edges(eijs)), k=2)
    G.remove_edges_from(edges_to_remove)

    edges_to_add = [(new_node_id, np.random.choice(list(G.nodes))) for _ in range(2)]
    G.add_edges_from(edges_to_add)


    return G

def prevresilience(G, t=4, m=2, method='random', rate=100,
                ntimes=10, output_list=True, printt=True,mode=True):
    """
    ...
    if mode:
        ...
        for new_node in range(t):
            ...
            for new_node in range(t):
                ...
                Gx = add_mutant_node(Gx, m, new_node, method)
                presilience.append(resilience(Gx, ntimes, rate, output_list=False))
